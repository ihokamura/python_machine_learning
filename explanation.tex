...
\section{系列データのモデル化ーリカレントニューラルネットワーク}
リカレントニューラルネットワーク(RNN)は系列データの学習に適したニューラルネットワークである.

\subsection{系列データ}
MLPやCNNの入力データは独立同分布であることを前提としている.
これに対して, RNNの入力データは系列データであり, データの順序に意味がある.
RNNでは, 過去のデータを記憶しておき, それに基づいて新しいデータを処理できるようなモデルを構築する.
シーケンスモデルはいくつかのカテゴリに分類される.
\begin{itemize}
    \item
    多対一：入力データは系列データであるが, 出力データは固定サイズである.
    例えば, テキストベースからクラスラベルを予測する感情分析モデルは多対一のシーケンスモデルである.


    \item
    一対多：入力データは固定サイズであるが, 出力データは系列データである.
    例えば, 画像からフレーズを予測する画像キャプショニングは一対多のシーケンスモデルである.

    \item
    多対多：入力データと出力データがともに系列データである.
    多対多のシーケンスは同期モデルと遅延モデルに分類される.
    例えば, 動画分類は同期モデルであり, 言語翻訳は遅延モデルである.
\end{itemize}

\subsection{リカレントニューラルネットワーク：シーケンスモデルの構築}
\subsubsection{RNNの構造とデータの流れを理解する}
MLPでは, 一つの入力データから一つの出力データが得られる.
これに対して, RNNでは複数の入力データから一つの出力データが得られる.
この関連は, ある時点の隠れ層が, その時点の入力データと一つ手前の時点の隠れ層と結合することにより実現される.
RNNという名称は, ネットワークのグラフ構造にリカレントエッジが現れることに由来する.

\subsubsection{RNNで活性化を計算する}
具体例として, 隠れ層を一つだけ持つRNNを考える.
時刻$t$における入力を$\bm{x}^{(t)}$, 隠れ層の出力を$\bm{h}^{(t)}$, 出力データを$\bm{y}^{(t)}$とする.
また, 入力層と隠れ層の間の重み行列を$\bm{W}_{xh}$, 隠れ層どうしの間の重み行列を$\bm{W}_{hh}$, 隠れ層と出力層の間の重み行列を$\bm{W}_{hy}$とする
\footnote{通常, これらの重み行列は時間に依存させない.}.
このとき, 入力データと出力データの関係は
\begin{align}
    \bm{h}^{(t)} &= \phi_{h}(\bm{W}_{xh}\bm{x}^{(t)} + \bm{W}_{hh}\bm{h}^{(t - 1)} + \bm{b_{h}}) \\
    \bm{y}^{(t)} &= \phi_{y}(\bm{W}_{hy}\bm{h}^{(t)} + \bm{b}_{y})
\end{align}
で与えられる.
時刻$T$におけるコスト関数はその時刻までのコスト関数の総和として与えられる.
\begin{equation}
    L^{(T)} = \sum_{t = 1}^{T} L^{(t)}
\end{equation}
この結果, 隠れ層どうしの間の重み行列に関する勾配はその時刻までのすべての入力データに依存する.
\begin{equation}
    \frac{\partial L^{(T)}}{\partial \bm{W}_{hh}}
    = \frac{\partial L^{(T)}}{\partial \bm{y}^{(t)}}\frac{\partial \bm{y}^{(t)}}{\partial \bm{h}^{(t)}}\sum_{t = 1}^{T}\frac{\partial \bm{h}^{(T)}}{\partial \bm{h}^{(t)}}\frac{\partial \bm{h}^{(t)}}{\partial \bm{W}_{hh}}
\end{equation}
ここで, 
\begin{equation}
    \frac{\partial \bm{h}^{(T)}}{\partial \bm{h}^{(t)}}
    = \prod_{\alpha = t + 1}^{T} \frac{\partial \bm{h}^{(\alpha)}}{\partial \bm{h}^{(\alpha - 1)}}
\end{equation}
であることに注意する.

\subsubsection{長期的な相互作用の学習}
前項の定式化を直接適用すると, 勾配消失または勾配発散の問題が発生する.
つまり, 偏導関数${\partial \bm{h}^{(\alpha)}}/{\partial \bm{h}^{(\alpha - 1)}}$の行列式が1より小さいと, $T$が大きくなるにつれて損失関数の勾配が0に近づく.
一方, その行列式が1より大きいと, $T$が大きくなるにつれて損失関数の勾配が発散する.
これらの問題に対処するため長短期記憶(LSTM)という方法が提唱された.

\subsubsection{LSTMのユニット}
LSTMでは, RNNのリカレントエッジにメモリセルと呼ばれる要素を追加する.
メモリセルはセル状態と呼ばれる値を持ち, 隠れ層間の入出力関係を制御する.
LSTMのメモリセルは内部的に3つのゲートを持っている.
\begin{itemize}
    \item
    忘却ゲート：メモリセルのセル状態をリセットさせる.
    \item
    入力ゲート：メモリセルのセル状態を更新する.
    \item
    出力ゲート：隠れ層の更新方法を制御する.
\end{itemize}
時刻$t$におけるセル状態を$\bm{C}^{(t)}$とすると, LSTMのメモリセルの機能は以下の関係式で表される.
\begin{align}
    \bm{f}^{(t)} &= \sigma(\bm{W}_{xf}\bm{x}^{(t)} + \bm{W}_{hf}\bm{h}^{(t - 1)} + \bm{b}_{f}) \\
    \bm{i}^{(t)} &= \sigma(\bm{W}_{xi}\bm{x}^{(t)} + \bm{W}_{hi}\bm{h}^{(t - 1)} + \bm{b}_{i}) \\
    \bm{g}^{(t)} &= \tanh(\bm{W}_{xg}\bm{x}^{(t)} + \bm{W}_{hg}\bm{h}^{(t - 1)} + \bm{b}_{g}) \\
    \bm{C}^{(t)} &= \bm{C}^{(t - 1)} \odot \bm{f}^{(t)} + \bm{i} \odot \bm{g}^{(t)} \\
    \bm{o}^{(t)} &= \sigma(\bm{W}_{xo}\bm{x}^{(t)} + \bm{W}_{ho}\bm{h}^{(t - 1)} + \bm{b}_{o}) \\
    \bm{h}^{(t)} &= \tanh \bm{C}^{(t)} \odot \bm{o}^{(t)}
\end{align}
