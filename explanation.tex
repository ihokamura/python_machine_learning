...
\begin{equation}
    \begin{cases}
        J(\bm{w}) = \prod_{i = 1}^{n} \phi(z^{(i)})^{y^{(i)}}(1 - \phi(z^{(i)}))^{1 - y^{(i)}} \\
        z^{(i)} = \bm{w}^{T}\bm{x}^{(i)}
    \end{cases}
\end{equation}
を最大化する重み$\bm{w}$を探索する.
ただし, 理論・実装の面から対数尤度を最小化する方が簡単であり, 通常は
\begin{equation}
    L(\bm{w}) = -\log J(\bm{w}) = -\sum_{i = 1}^{n} \left[y^{(i)}\log \phi(z^{(i)}) + (1 - y^{(i)})\log(1 - \phi(z^{(i)}))\right]
\end{equation}
を最小化する重み$\bm{w}$を探索する.
\begin{align}
...
に注意すると, 
\begin{align}
    \frac{\partial L}{\partial w_{j}}(\bm{w})
